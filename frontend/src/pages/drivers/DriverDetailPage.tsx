...
export default function DriverDetailPage() {
  ...
  const [driver, setDriver] = useState<Driver & { deliveries?: Delivery[] } | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (!id) return;
    driversApi.getOne(id).then((r) => setDriver(r.data)).catch(() => navigate('/drivers')).finally(() => setLoading(false));
  }, [id]);
    ...
    <div className="page-content">
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <button className="ab-btn ab-btn-outline" onClick={() => navigate('/drivers')}>
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="15 18 9 12 15 6"/></svg>
            Back
          </button>
          <div>
            <div className="section-title">{driver.fullName}</div>
            <div style={{ fontFamily: 'IBM Plex Mono,monospace', fontSize: 11, color: 'var(--steel)', marginTop: 2 }}>{driver.driverCode} · {driver.phone}</div>
          </div>
        </div>
        <span className={`pill ${driver.status === 'ACTIVE' ? 'pill-green' : 'pill-steel'}`} style={{ fontSize: 12, padding: '4px 12px' }}>{driver.status}</span>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr', gap: 16, alignItems: 'start' }}>
        ...
        <div className="panel" style={{ padding: 20 }}>
          <div style={{ fontFamily: 'Oswald,sans-serif', fontSize: 12, fontWeight: 600, letterSpacing: '0.05em', color: 'var(--steel)', marginBottom: 14, paddingBottom: 10, borderBottom: '1px solid var(--rule)' }}>DRIVER DETAILS</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
            {[
              { label: 'Phone', val: driver.phone },
              { label: 'License #', val: driver.licenseNumber || '—' },
              { label: 'Address', val: driver.address || '—' },
              { label: 'Status', val: driver.status },
              { label: 'Since', val: formatDate(driver.createdAt) },
            ].map(({ label, val }) => (
