...
import type { Supplier, Purchase, SupplierPayment, GasReceiving } from '@/types';

type Tab = 'overview' | 'purchases' | 'payments' | 'receivings' | 'ledger';
...
export default function SupplierDetailPage() {
  ...
  const [supplier, setSupplier] = useState<Supplier & { purchases?: Purchase[]; supplierPayments?: SupplierPayment[]; gasReceivings?: GasReceiving[] } | null>(null);
  ...
  const handleExportStatement = async () => {
    ...
    await exportToPdf(`Statement-${supplier.supplierCode}`, `Supplier Statement — ${supplier.supplierName}`, head, body);
  };
  ...
  const receivings = supplier.gasReceivings || [];
  ...
  const totalVarianceQty = receivings.reduce((s, r) => s + r.variance, 0);
  const shortDeliveries = receivings.filter((r) => r.variance < 0).length;
  ...
  const tabs: { key: Tab; label: string }[] = [
    ...
    { key: 'receivings', label: `Receivings (${receivings.length})` },
  ...
  return (
    <div className="page-content">
      ...
      {activeTab === 'receivings' && (
        <div className="panel" style={{ padding: 20 }}>
          {receivings.length === 0 ? (
            <div style={{ padding: 40, textAlign: 'center', color: 'var(--steel)', fontSize: 12, fontFamily: 'IBM Plex Mono,monospace' }}>No gas receiving records</div>
          ) : (
            <>
              <div style={{ marginBottom: 14, padding: '10px 14px', background: 'var(--paper)', border: '1px solid var(--rule)', borderRadius: 'var(--radius)', display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
                <span style={{ color: 'var(--steel)' }}>Total Variance · {shortDeliveries} short deliver{shortDeliveries === 1 ? 'y' : 'ies'}</span>
                <span style={{ fontFamily: 'IBM Plex Mono,monospace', fontWeight: 700, color: totalVarianceQty < 0 ? 'var(--red-risk)' : 'var(--green-ok)' }}>{totalVarianceQty > 0 ? '+' : ''}{totalVarianceQty.toFixed(1)} KG</span>
              </div>
              <div style={{ overflowX: 'auto' }}>
                <table className="data-table">
                  <thead>
                  ...
                  <tbody>
                    {receivings.map((r) => (
                      <tr key={r.id}>
                        ...
                        <td><span style={{ fontSize: 12, color: 'var(--steel)' }}>{r.tank?.tankName || '—'}</span></td>
                        <td style={{ textAlign: 'right' }}><span style={{ fontFamily: 'IBM Plex Mono,monospace', fontSize: 12 }}>{r.expectedQuantity} {r.unit}</span></td>
                        <td style={{ textAlign: 'right' }}><span style={{ fontFamily: 'IBM Plex Mono,monospace', fontSize: 12 }}>{r.receivedQuantity} {r.unit}</span></td>
                        <td style={{ textAlign: 'right' }}>
                          <span style={{ fontFamily: 'IBM Plex Mono,monospace', fontSize: 12, fontWeight: 700, color: r.variance < 0 ? 'var(--red-risk)' : r.variance > 0 ? 'var(--green-ok)' : 'var(--steel)' }}>
