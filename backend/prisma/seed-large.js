...
function rFloat(min, max, d = 0) {
    const v = Math.random() * (max - min) + min;
    return d ? parseFloat(v.toFixed(d)) : Math.round(v);
}
function pick(arr) { return arr[Math.floor(Math.random() * arr.length)]; }
function uid() { return (0, crypto_1.randomUUID)().replace(/-/g, '').slice(0, 24); }
function sortByDate(arr) { return arr.sort((a, b) => a.date.getTime() - b.date.getTime()); }
...
function randomCode(pfx, n) { return `${pfx}-${String(n).padStart(4, '0')}`; }
async function main() {
    ...
    const customers = [];
    const custTypes = ['RETAIL', 'RETAIL', 'RETAIL', 'DEALER', 'DEALER', 'COMMERCIAL', 'INDIVIDUAL'];
    for (let i = 1; i <= 500; i++) {
    ...
    for (let i = 0; i < saleRecords.length; i += BATCH) {
        ...
        if ((i + BATCH) % 2000 === 0)
            console.log(`   → ${i + BATCH} sales done...`);
    }
    console.log('💳 Creating 8,000 customer payments...');
    const custPayments = [];
    const saleSample = saleRecords.filter((_, idx) => idx % 2 === 0);
...
//# sourceMappingURL=seed-large.js.map